...
- A new `lix` CLI tool has been added, gated behind `--extra-experimental-features lix-command`. Running `lix foo` will invoke `lix-foo` from your `$PATH`, similar to how Git and Cargo integrate with other tools. The `lix` CLI does not provide any commands itself yet. Thanks to [raito][raito].
