...
- A new `lix` CLI tool has been added. Running `lix foo` will invoke `lix-foo` from your `$PATH`, similar to how Git and Cargo integrate with other tools. The `lix` CLI does not provide any commands itself yet. Thanks to [raito][raito].
...
- Canceling a build with Ctrl-C on macOS no longer leaves `nix-daemon` processing lingering.
