...
- More deprecations of old language features
  ...
  The Nix language is replete with features that by now are mostly problematic, such as merges of `rec` sets (which sometimes did not create rec sets!), dynamic attributes in `rec` sets (which never behaved as one would have expected), warnings for probable tokenization issues (which mean `[ 00.123 ]` is a list with *two* elements), and many more.
